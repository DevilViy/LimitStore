...
import {sendSmsBack,checkCodeBack} from '../../services/api'
...
class Forget extends Component {
    ...
    jumpTwo=()=>{
        ...
        const {phoneNumber,checkNumber,realNameText} = this.state
        if(phoneNumber===''||checkNumber===''){
            Toast.info('请完整的填完信息')
        }
        else{
            checkCodeBack({phone:phoneNumber,code:checkNumber,real_name:realNameText}).then((resp)=>{
    ...
    clickGetCheck=(shouldStartCountting)=>{
        ...
        if(PHONE_REGEX.test(phoneNumber)) {
            ...
            sendSmsBack({phone:phoneNumber}).then((resp)=>{
                console.log('resp',resp)
                if(resp.status===1){
                    // 成功
    ...
    render() {
        const { phoneNumber, nation, checkNumber, realNameText } = this.state
        return (
